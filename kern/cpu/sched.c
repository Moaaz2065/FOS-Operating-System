...
void sched_init_BSD(uint8 numOfLevels, uint8 quantum)
{
	...
	//priorities = kmalloc(numOfLevels*(sizeof(uint8)));
...
int ExpectedQueueIdx(int priority)
{
	for(int i = 0 ; i < num_of_ready_queues ; i++)
	{
		if(priority <= priorities[i])
			return (num_of_ready_queues-i-1);
	}
	return 0;
}
...
struct Env* fos_scheduler_BSD()
{
	...
	for(int i = num_of_ready_queues-1 ; i >= 0 ; i--)
	{
		...
		ret = dequeue(&env_ready_queues[i]);
		break;
	}
	if(ret == NULL)
	{
	...
	if(scheduler_method == SCH_BSD)
	    {
	        ...
	        fixed_point_t x,y;
	        int tick = ticks*quantums[0]/1000;
	        int diff_tick = (ticks+1)*quantums[0]/1000;
	        x = fix_frac(59,60);
	        y = fix_frac(1,60);
	        ...
	        struct Env* env = NULL;
	        for (int i = num_of_ready_queues-1; i>=0;i--){
	            size +=queue_size(&env_ready_queues[i]);

	        }
	        if(curenv!= NULL)
	            size++;
	        fixed_point_t ready = fix_int(size);
	        if(tick!= diff_tick){
	            loadAVG = fix_add(fix_mul(x,loadAVG),fix_mul(y,ready));
	            for(int i = num_of_ready_queues-1; i>=0;i--){
	            LIST_FOREACH(env,&env_ready_queues[i]){
	            fixed_point_t recent = fix_int(0);
	            recent = fix_mul(fix_div(fix_scale(loadAVG,2),fix_add(fix_scale(loadAVG,2),one)),recent);
	            recent = fix_add(recent,fix_int(env->nice_value));
	                }
	            }
	        }
	        if(tick%4 ==0){
	            for(int i = num_of_ready_queues-1; i>=0;i--){
	            LIST_FOREACH(env,&env_ready_queues[i]){
	            env->priority = PRI_MAX - (fix_trunc(env->recentCPU)/4) - (env->nice_value*2);
	            if(env->priority < 0)
	                env->priority = 0;
	            else if (env->priority>63)
	                env->priority = 63;
	                }
	            }
	            for(int i = num_of_ready_queues-1; i>=0;i--){
