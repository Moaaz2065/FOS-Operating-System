...
void sched_print_all()
{
	...
	for (int i = 0 ; i < num_of_ready_queues ; i++)
	{
		if (!LIST_EMPTY(&(env_ready_queues[i])))
		{
			...
			LIST_FOREACH(ptr_env, &(env_ready_queues[i]))
			{
				cprintf("	[%d] %s %d %d\n", ptr_env->env_id, ptr_env->prog_name,ptr_env->priority,fix_trunc(ptr_env->recentCPU));
			}
		}
		else
		{
...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	{
		e->priority = PRI_MAX - fix_trunc(fix_unscale(e->recentCPU,4)) - (e->nice_value*2);
		e->priority = (e->priority <= PRI_MAX ? e->priority : PRI_MAX);
		e->priority = (e->priority >= PRI_MIN ? e->priority : PRI_MIN);
...
int env_get_recent_cpu(struct Env* e)
{
	...
	//Your code is here
	//Comment the following line
	return 100*fix_trunc(e->recentCPU);
}
