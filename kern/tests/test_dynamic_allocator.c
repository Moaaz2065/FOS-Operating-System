...
void test_realloc_block_FF()
{
	...
	void * va ;
	//our assumed test cases that is not in the partial tests
		/*uint32 space[3]={Mega,Mega,Mega};

			va =alloc_block_FF(space[0]-sizeOfMetaData());
			struct BlockMetaData *block;

				LIST_FOREACH (block,&(Blocks)){
					cprintf("1- size=%d free=%d pptr=%pl\n",block->size,block->is_free,block);
				}
			va =alloc_block_FF(space[1]-sizeOfMetaData());
			LIST_FOREACH (block,&(Blocks)){
					cprintf("2-size=%d free=%d pptr=%p\n",block->size,block->is_free,block,block);
				}
			va =alloc_block_FF(space[2]-sizeOfMetaData());
			LIST_FOREACH (block,&(Blocks)){
					cprintf("3-size=%d free=%d pptr=%p \n",block->size,block->is_free,block,block);
				}

			free_block((void*)(KERNEL_HEAP_START+2*Mega+sizeOfMetaData()));
			realloc_block_FF((void*)(KERNEL_HEAP_START+1*Mega+sizeOfMetaData()),2*Mega-sizeOfMetaData());
			//realloc_block_FF((void*)KERNEL_HEAP_START+sizeOfMetaData()+1*Mega-2,2*Mega-sizeOfMetaData());


			LIST_FOREACH (block,&(Blocks)){
						cprintf("4-size=%d free=%d pptr=%p \n",block->size,block->is_free,block,block);
					}
			return;*/
	//====================================================================//
