...
{
	LIST_INIT(&foundCommands);
	char* _command=arguments[0];
	char temp[strlen(arguments[0])];
	//_command = str2lower(temp,_command);
	for(int i=0 ; i< NUM_OF_COMMANDS;i++)
	{

		if(strcmp(commands[i].name,_command) == 0 &&number_of_arguments-1==commands[i].num_of_args){

			return i;

		}
		else if(strcmp(commands[i].name,_command) == 0 && commands[i].num_of_args == -1)
		{
			if(number_of_arguments >= 1)
			{
				return i;
	...
	for(int i=0;i<NUM_OF_COMMANDS;i++){
		int j=0;
		int str_size=strlen(commands[i].name);
		for(int s=0; j < strlen(_command) && s < str_size;s++){
			if(_command[j]==commands[i].name[s]){
				j++;
			}
		}
		if(j==strlen(_command)){
