...
void* sys_limit(){
	...
	return (void*)env->u_limit;
}

int is_marked(uint32 va)
{
	uint32 *page = NULL;
	int ret = get_page_table(curenv->env_page_directory,va,&page);
	if(ret) {
		page = create_page_table(curenv->env_page_directory,va);
	}
	return ((page[PTX(va)]&PERM_AVAILABLE) == 0);
}
...
void* sys_sbrk(int increment)
{
	...
	struct Env* env = curenv; //the current running Environment to adjust its break limit
if(increment==0)
	return (void*)curenv->u_brk;

else if(increment>0){
	if(curenv->u_brk+increment > curenv->u_limit)
		return (void *)-1;

	uint32 old_brk=curenv->u_brk;
	dec -= increment;
	if(ROUNDUP(old_brk,PAGE_SIZE)-old_brk >= increment)
	{
		curenv->u_brk += increment;
		return (void *)old_brk;
	}
	increment=ROUNDUP(increment,PAGE_SIZE);

	curenv->u_brk+=increment;
	for(uint32 i = old_brk ; i < curenv->u_brk ; i+=PAGE_SIZE)
	{
		uint32 *page;
		int ret = get_page_table(curenv->env_page_directory,i,&page);
		if(ret) page = create_page_table(curenv->env_page_directory,i);
	...
	if(old_brk-increment < USER_HEAP_START)
		return (void *)-1;
	old_brk=curenv->u_brk;

	if(dec+increment >= PAGE_SIZE) {
		uint32 *page;
        int ret = get_page_table(curenv->env_page_directory,old_brk-(PAGE_SIZE-dec),&page);
		if(!ret) page[PTX(old_brk-(PAGE_SIZE-dec))] &= ~PERM_AVAILABLE;
		//Free Page
		sheel_ws_bsor3a(curenv,old_brk-(PAGE_SIZE-dec));
		unmap_frame(curenv->env_page_directory,old_brk-(PAGE_SIZE-dec));
		pf_remove_env_page(curenv,old_brk-(PAGE_SIZE-dec));
		dec += increment;
		dec -= PAGE_SIZE;
	}
	curenv->u_brk -= increment;
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_sbrk:
		...
		case SYS_free_user_mem:
			if(a1==(uint32)NULL||a1+a2>USER_LIMIT){
		...
		case SYS_allocate_user_mem:
			if(a1==(uint32)NULL||a1>=USER_LIMIT){
				sched_kill_env(curenv->env_id);
				return 0;
				break;
